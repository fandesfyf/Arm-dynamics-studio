...
- [面板说明](#面板说明)
  ...
  - [右侧：可视化](#右侧可视化)
  - [底部：曲线](#底部曲线)
- [CSV 数据导入导出](#csv-数据导入导出)
  - [运动目标轨迹（插值队列）](#运动目标轨迹插值队列)
  - [仿真时序曲线（底部面板）](#仿真时序曲线底部面板)
- [典型工作流](#典型工作流)
- [快捷键](#快捷键)
- [技术栈](#技术栈)
- [环境要求](#环境要求)
...
- **关节限速**（rad/s）
- **线性** / **三次样条** 插值曲线
- **目标队列**：查看、删除、**导入 CSV** / **导出 CSV**（见 [运动目标轨迹](#运动目标轨迹插值队列)）
...
- **清空曲线数据**：重置 recorder。
- **录制窗口**：环形缓冲保留最近 N 秒（侧栏可调「保留窗口 (s)」）。
- **导出**：打开导出对话框，按需选择曲线类型与关节（见 [仿真时序曲线](#仿真时序曲线底部面板)）。

支持滚轮缩放、拖拽平移；停止仿真后历史曲线仍保留，直至手动清空或窗口过期。

## CSV 数据导入导出

应用内有两类 CSV，用途不同，不要混用。

| 类型 | 入口 | 内容 | 典型用途 |
|------|------|------|----------|
| **运动目标轨迹** | 右栏 **控制** → 插值模式 → 目标队列 | 关节空间路标点（界面显示 FK 末端位置） | 保存/复用插值运动序列 |
| **仿真时序曲线** | 底部 **曲线** 工具栏 **导出** | 仿真过程中的 time series | 分析跟踪误差、对比指令与实际 |

### 运动目标轨迹（插值队列）

**位置**：右栏 **控制** 面板，切换到 **插值** 模式，展开 **目标队列 (N)**。

| 操作 | 说明 |
|------|------|
| **添加目标** `K` | 关节模式：直接记录关节角；末端模式：先 IK 再记录关节角 |
| **导入 CSV** | 从文件加载路标点，**替换**当前队列 |
| **导出 CSV** | 下载当前队列；无目标时按钮禁用 |
| **清空** | 删除全部路标点 |
| **执行** `F` | 按队列顺序插值运动 |

**使用步骤（导出）**

1. 插值模式下多次 **添加目标** 或调整滑条后入队。
2. 展开目标队列，点击 **导出 CSV**。
3. 浏览器下载 `<模型名>_motion_targets.csv`，顶栏提示已导出数量。

**使用步骤（导入）**

1. 准备好符合格式的 CSV（可用本应用导出的文件作模板）。
2. 点击 **导入 CSV**，选择文件。
3. 队列被导入内容**整体替换**；仅读取关节列，导入后由 FK 计算末端位姿用于列表与视口标记。
4. 检查队列列表（FK 末端坐标）与视口标记，确认无误后 **执行** `F`。

**CSV 列说明**（首行为表头，纯关节空间格式）：

| 列名 | 含义 |
|------|------|
| `index` | 序号（从 1 起，导入时仅作参考） |
| `q_<关节名>` | 各关节目标角（rad），列名与当前模型关节名一致，如 `q_zarm_l1_joint` |

示例：

```text
index,q_zarm_l1_joint,q_zarm_l2_joint,q_zarm_l3_joint,...
1,0.12,-0.45,1.02,...
2,0.20,-0.30,0.95,...
```

> 队列列表显示的是各帧关节角经 **FK** 得到的末端位置，与 CSV 中是否含末端列无关。  
> 导入的模型须与导出时关节名一致；仍兼容旧版含 `ee_*` 列的 CSV，但末端列会被忽略、以 FK 为准。

### 仿真时序曲线（底部面板）

**位置**：底部 **曲线** 面板工具栏 **导出** 按钮（需已有录制数据，否则禁用）。

**前置条件**

- 至少运行过一次仿真并有采样（顶栏可见「采样 N」或曲线有数据）。
- **停止仿真后数据仍保留**，可先跑仿真再导出；**暂停录制** 只影响是否继续写入，不删除已有数据。

**导出步骤**

1. 展开底部曲线面板，切换到需要的页签（位置/速度/力矩/末端）预览。
2. 可选：用页签上方关节筛选勾选要关注的关节；调整 **保留窗口 (s)** 仅影响屏幕显示与「最近窗口」导出范围。
3. 点击 **导出**，在对话框中配置：
   - **曲线类型**：位置、速度、力矩、末端（可多选）
   - **关节**：勾选要导出的关节（末端指标不受关节勾选限制）
   - **时间范围**：全部录制数据，或最近 N 秒（N = 当前录制窗口，默认 30s）
4. 确认 **导出**，下载 `<模型名>.csv`（如 `test_arm.csv`）。

**CSV 列说明**（按所选类型组合，6 位小数）：
...
| `<关节>_vel` | 实际关节速度 |
| `<关节>_torque` | 实际关节力矩 |
| `<关节>_pos_desired` | 指令关节位置（选「位置」且录制含指令时） |
| `<关节>_vel_desired` | 指令关节速度（选「速度」且录制含指令时） |
| `<关节>_torque_cmd` | 指令力矩（选「力矩」且录制含指令时） |
| `ee_x` `ee_y` `ee_z` | 末端位置（选「末端」时） |

列顺序：`time` → 位置组 → 速度组 → 力矩组 → 指令位置 → 指令速度 → 指令力矩 → 末端。格式与旧版 Python 桌面仿真工具对齐，便于离线对比分析。

**与曲线显示的关系**

| 界面元素 | 作用 |
|----------|------|
| 页签（位置/速度/…） | 仅影响**屏幕**显示哪类曲线 |
| 关节勾选（曲线工具栏） | 仅影响**屏幕**显示哪些关节 |
| 导出对话框 | 独立选择导出列，可与当前页签不同 |
...
3. 左栏 **▶ 开始仿真** → 右栏 **执行**。
4. 底部曲线查看跟踪效果 → **导出** 仿真时序 CSV。

### 轨迹 CSV 复用（插值模式）

1. 调整好路标点后 **导出 CSV** 备份。
2. 下次打开同模型 → **导入 CSV** 恢复队列 → **执行** `F`。
3. 或在其他会话中编辑 CSV 后导入，再执行插值。
...
| 按键 | 作用 |
|------|------|
| `Space` | 就绪时开始仿真；运行中停止仿真（焦点不在输入框/按钮上时） |
| `K` | 插值模式下添加运动目标到队列（等同「添加目标」） |
| `F` | 插值模式下执行运动目标（等同「执行」） |
